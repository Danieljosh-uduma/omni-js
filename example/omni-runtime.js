...
function transformReactivitySyntax(scriptContent) {
  let masked = scriptContent.replace(/state\s+\?([a-zA-Z0-9_]+)/g, "let __OMNI_STATE_$1").replace(/\?([a-zA-Z0-9_]+)/g, "__OMNI_USE_$1");
  let ast;
  try {
    ...
    FunctionDeclaration(node) {
      if (node.id && node.id.name) {
        replacements.push({
          ...
          text: `context.${node.id.name} = function`
        });
      }
    }
  });
...
async function walkAndTransform(node, context, state, components) {
  ...
  Array.from(node.attributes).forEach((attr) => {
    ...
    if (name.startsWith("bind-")) {
      ...
      const signalName = value.replace("?", "");
      if (context[signalName]) {
        effect(() => {
          if (prop === "text")
            el.textContent = context[signalName].value;
          else if (prop === "value") {
            if (el.value !== context[signalName].value) {
              el.value = context[signalName].value;
            }
          } else
            el.setAttribute(prop, context[signalName].value);
        });
        if (prop === "value") {
          el.addEventListener("input", (e) => {
            context[signalName].value = e.target.value;
          });
        }
      }
      return;
  ...
  if (routePath !== null) {
    ...
    effect(() => {
      const current2 = routerSignal.value;
      const isMatch = routePath === current2 || routePath === "/" && (current2 === "/" || current2 === "");
